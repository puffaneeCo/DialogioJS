/*! Puffanee Dialoger - UI Helper for JavaScript working with jQuery | (c) Puffanee | github.com/puffanee/Dialoger/license */
(function ($) {
  if (!$) {
    console.error("[DIALOGER ALERT] jQuery not loaded.");
    return;
  }

  $("body").prepend(
    `<div class="Dialoger__CustomAlerts" id="dlgalertcontainer"><div class="Alert__List" id="dlgalertpush"></div></div>`
  );

  const $AlertContainer = $("#dlgalertcontainer");
  const $Alerts = $("#dlgalertpush");
  ...
  const styles = ["Default", "Danger", "Success", "Warning", "Information"];

  $.dlgalert = function (style = 0, message = "", setIcon = true) {
    if (!message) {
      console.error("[DIALOGER ALERT] Empty message");
      return;
    }

    const alertStyle = styles[style] || "Default";
    const icon = setIcon ? icons[style] || icons[0] : false;

    const html = `
      <div class="Dialoger__CustomAlert ${alertStyle}" id="dlgalert">
        ${icon ? `<div class="Alert__Icon">${icon}</div>` : ""}
  ...
  $.dlgalert.setLocation = function (location = "TopCenter") {
    const valid = [
      "TopCenter",
      "LeftTop",
      "RightTop",
    ...
    if (valid.includes(location)) {
      $AlertContainer
        .removeAttr("class")
        .addClass("Dialoger__CustomAlerts")
        .addClass(location);
    }
  };

  $(document).on("click", ".Dialoger__CustomAlert", function () {
    $(this).remove();
  });
})(window.jQuery);
